...
def save_to_file(hashed_password, salt):
    try:
        with open("hashedpassword.txt","a") as file:
            file.write(f"Salt:{salt.hex()} Hashed: {hashed_password}\n")
        messagebox.showinfo("Password Saved", "The password has been saved to 'hashedpassword.txt")
